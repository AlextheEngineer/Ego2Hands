...
def construct_model_seg(args, config, energy_status, seq_i = -1, model_i = 1):
    ...
    if args.pretrained or args.adapt or args.eval or args.custom:
    ...
    if not args.train_all:
        ...
        if not args.adapt:
            ...
            for seq_i in range(0, config.num_seqs):
                ...
                if os.path.exists(os.path.join(model_save_dir, model_name)):
                    print("Skip training for model {}, sequence {}".format(model_i, seq_i))
                    continue
        ...
        if not args.adapt:
            for model_i in range(args.num_models):
                ...
                if os.path.exists(os.path.join(model_save_dir, model_name)):
                    print("Skip training for model {}".format(model_i))
                    continue
...
def test_model(args):
    ...
    if not args.test_all:
        if not args.custom:
            ...
            model_seg = construct_model_seg(args, config, energy_status = energy_status)
            test_ego2hands_seg(model_seg, -1, args, config, energy_status = energy_status)
